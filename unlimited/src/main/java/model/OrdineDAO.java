...
public class OrdineDAO extends AbstractDAO<OrdineBean> {
	...
	public synchronized void doSave(OrdineBean bean) throws SQLException {
		...
		String query = "INSERT INTO " + OrdineDAO.TABLE_NAME + 
					" (cliente,numCarta,citta,CAP,via,civico,provincia,nazione,dataAcquisto,costoTotale) VALUES (?,?,?,?,?,?,?,?,?,?);";
...
//		se ne parla dopo TODO
	...
	public synchronized boolean doUpdate(OrdineBean bean) throws SQLException {
		...
		String query = "UPDATE " + OrdineDAO.TABLE_NAME + " SET "
				+ "numOrdineProgressivo = ?, cliente = ?, numCarta = ?, citta = ?,"
		...
		try {
			...
			statement.setString(6, bean.getVia());
			statement.setString(7, bean.getCivico());
			statement.setString(8, bean.getProvincia());
			statement.setString(9, bean.getNazione());
			statement.setString(10, bean.getDataAcquisto());
			statement.setBigDecimal(11, bean.getCostoTotale());
